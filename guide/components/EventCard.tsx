...
export function EventCard({ event, compact }: Props) {
  ...
  return (
    <Link href={`/event/${event.id}`} asChild>
      <Pressable style={({ pressed }) => [styles.card, pressed && styles.pressed]} accessibilityRole="button">
        ...
        {!compact && <Text style={styles.summary} numberOfLines={2}>{event.summary}</Text>}
        {venue ? (
